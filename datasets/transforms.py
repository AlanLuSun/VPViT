...
class Resize(PreprocessAbstract):
    def __init__(self, longer_length=None, resample=PIL.Image.BILINEAR, dst_shape=None):
        self.longer_length = longer_length
        self.resample = resample  # PIL.Image.BILINEAR, PIL.Image.CUBIC, PIL.Image.BICUBIC
        # print(self.__class__.__name__)
        self.dst_shape = dst_shape  # if dst_shape is not None, we resize into dst_shape directly
        assert (longer_length is not None) or (dst_shape is not None)

    def __call__(self, image, anns, meta):
        w, h = image.size  # image should be PIL image
        # undistorted resize along long side
        if self.longer_length is not None:
            ...
            target_shape = self.dst_shape
            scale = w*1.0 / self.dst_shape[0] if w >= h else h*1.0 / self.dst_shape[1]
...
class RandomErasingKeypoints(PreprocessAbstract):
    ...
    def __init__(self, erase_kp_range=None, prob_erase_kp=0.1, occlude_type='graybox', im_save=False, prob=0.5, sl=0.0025, sh=0.01, r1=0.7, mean=[0.485, 0.456, 0.406, 0]):
        self.erase_kp_range = erase_kp_range
        self.prob_erase_kp = prob_erase_kp
        self.occlude_type = occlude_type  # 'graybox', 'bgbox', 'avgpixel'
        self.im_save = im_save

        self.probability = prob
        self.mean = (np.array(mean)*255).round().astype(np.uint8)
        self.sl = sl
        self.sh = sh
        self.r1 = r1

    def __call__(self, image, anns, meta):
        keypoints = anns['keypoints']  # N x 3, each row (x, y, is_valid)
        bbox = anns['bbox']  # (xmin, ymin, w, h)
        num_valid_kps = np.sum(keypoints[:,2])
        # num_erase_kps = round(num_valid_kps * self.prob_erase_kp)
        if self.erase_kp_range == None:
            self.erase_kp_range = [1] * len(keypoints)
        # t1 = time.time()
        image_prob = random.uniform(0, 1)
        if (image_prob > self.probability) or (self.prob_erase_kp <= 0) or (num_valid_kps <= 0):
            pass  # end to return
        else:
            ...
            sample_success = True
            # get shuffled BG pixels for occlusion
            if self.occlude_type == 'bgbox':
                im_h, im_w = image.shape[:2]
                saliency_map = image[:,:,3]

                # too slow to sample keypoints as need to process whole saliency map
                # bg_indicator = saliency_map.reshape(-1) < 0.4*255  # bg thresh
                # bg_inds = np.arange(0, im_h*im_w)
                # if sum(bg_indicator) <= 100:  # less than 100 pixels
                #     sample_success = False
                # else:
                #     bg_inds = bg_inds[bg_indicator]  # bg indexes used to sample bg points (bg occlusion box)
                #
                #     t5 = time.time()
                #     print("t2: ", t5 - t1)
                #
                #     random.shuffle(bg_inds)


                # fast sample method (sample points from background for constructing occlusion of background crop)
                sample_success, sampled_kps = sample_points_within_FG(len(keypoints), (0,0,im_w,im_h), 'random', 255-saliency_map, thresh=0.4*255)
            ...
            for i in range(len(keypoints)):
                if (self.erase_kp_range[i] == 1) and (keypoints[i, 2] == 1) and (sample_success==True):  # erase_kp_range & valid kp
                    p = random.uniform(0, 1)
                    if p <= self.prob_erase_kp:
                        ...
                        for attempt in range(100):
                            ...
                            if w < bbox[2] and h < bbox[3]:
                                if self.occlude_type == 'graybox':
                                    for c in range(image.shape[2]):
                                        image[y1 - h//2:y1 + h//2, x1 - w//2:x1 + w//2, c] = self.mean[c]
                                elif self.occlude_type == 'bgbox':
                                    '''
                                    there is no check to ensure [y1 - h // 2:y1 + h // 2, x1 - w // 2:x1 + w // 2] to be 
                                    within image, so sometimes the crop patch is smaller than (2* h//2, 2*//w)
                                    '''
                                    tgt_patch = image[y1 - h // 2:y1 + h // 2, x1 - w // 2:x1 + w // 2]
                                    p_h, p_w = tgt_patch.shape[:2]

                                    bgbox_sample_flag = False
                                    # # sample a bg point
                                    # for trial in range(100):
                                    #     # num_sampled = np.random.randint(0, len(bg_inds))
                                    #     # ind_sampled = bg_inds[num_sampled]
                                    #     ind_sampled = bg_inds[trial]
                                    #     x_s = ind_sampled % im_w  # regard this point as top-left point
                                    #     y_s = ind_sampled // im_w
                                    #     # print(trial)
                                    #     if (y_s) >= 0 and (y_s + p_h) <= im_h and (x_s) >= 0 and (x_s + p_w) <= im_w:
                                    #         bg_patch = (image[y_s:y_s + p_h, x_s:x_s + p_w])
                                    #         bg_patch[:,:,-1] = 0  # set 0 to saliency channel (RGBD)
                                    #
                                    #         # ww, hh = bg_patch.shape[:2]
                                    #         # print('p1: {}, {}'.format(ww, hh))
                                    #         # tt = image[y1 - h // 2:y1 + h // 2, x1 - w // 2:x1 + w // 2]
                                    #         # ww2, hh2 = tt.shape[:2]
                                    #         # print('p2: {}, {}'.format(ww2, hh2))
                                    #         # if (ww != ww2) or (hh!=hh2):
                                    #         #     print('error')
                                    #
                                    #         image[y1 - h // 2:y1 + h // 2, x1 - w // 2:x1 + w // 2] = bg_patch
                                    #         bgbox_sample_flag = True
                                    #         break


                                    (x_s, y_s) = sampled_kps[i]  # regard this point as top-left point
                                    # print(trial)
                                    if (y_s + p_h) > im_h:  # boundary process
                                        y_s -= p_h
                                    if (x_s + p_w) > im_w:
                                        x_s -= p_w
                                    if (y_s) >= 0 and (y_s + p_h) <= im_h and (x_s) >= 0 and (x_s + p_w) <= im_w:
                                        ...
                                        image[y1 - h // 2:y1 + h // 2, x1 - w // 2:x1 + w // 2, :3] = bg_patch[:,:,:3]  # set RGB
                                        image[y1 - h // 2:y1 + h // 2, x1 - w // 2:x1 + w // 2, 3] = 0  # set 0 to saliency channel (RGBD)
                                        bgbox_sample_flag = True

                                elif self.occlude_type == 'avgpixel':
                                    tgt_patch = image[y1 - h // 2:y1 + h // 2, x1 - w // 2:x1 + w // 2]
                                    p_h, p_w = tgt_patch.shape[:2]
                                    '''
                                    sometimes p_h or p_w equal to 0 due to no guaranttee tgt_patch within image
                                    (need to fix this when have time)
                                    '''
                                    if p_h > 0 and p_w > 0:
                                        avg_pixel = tgt_patch.reshape(-1, 4).mean(axis=0)
                                        avg_pixel[-1] = 0  # set 0 to saliency channel (RGBD)
                                        image[y1 - h // 2:y1 + h // 2, x1 - w // 2:x1 + w // 2] = avg_pixel
                                else:
                                    raise NotImplementedError
                                break  # break the loop for attempt
        ...
        if self.im_save:
            root = './episode_images/occluded_ims/'
            if os.path.exists(root) == False:
                os.makedirs(root)
            # separate RGB image and saliency image
            R, G, B, saliency_image = image.split()
            im_tmp = Image.merge('RGB', (R, G, B))
            im_tmp.save(root + '{}.jpg'.format(meta['im_index']))
            saliency_image.save(root + '{}s.jpg'.format(meta['im_index']))

        return image, anns, meta

from utils import sample_keypoints_within_rect
def sample_points_within_FG(nkps=20, bbx=(0, 0, 384, 384), sample_method='random', FG_map=None, thresh=0.4 * 255):
    '''
    Our idea is to sample points within bbox first and then filter those points lying on foreground. (this way is more efficient
    than directly sampling on many foreground pixels)
    bbx (xmin, ymin, w, h)
    FG_map: H x W
    thresh: saliency thresh

    return points with size nkps x 2
    '''
    ...
    FG_map_array = FG_map.reshape(-1)
    im_h, im_w = FG_map.shape
    for i in range(30):  # try 30 times
        # # correct if some bbox is out of image
        # bbx[0], bbx[1] = max(bbx[0], 0), max(bbx[1], 0)
        # bbx[2], bbx[3] = min(bbx[2], w - bbx[0]), min(bbx[3], h - bbx[1])

        pts = sample_keypoints_within_rect(N, 0, 0, bbx, sample_method)  # N x 2
        pts_inds = pts[:, 1] * im_w + pts[:, 0]  # N
        sal_values = FG_map_array[pts_inds]  # N
        indicator = sal_values >= thresh  # N
...
class RandomErasingFGPatches(PreprocessAbstract):
    """ Randomly selects a rectangle region in an image and erases its pixels.
        'Random Erasing Data Augmentation' by Zhong et al.
        See https://arxiv.org/pdf/1708.04896.pdf
    Args:
    ...
    def __init__(self, num_patch_l=1, num_patch_h=3, im_save=False, prob=0.5, sl=0.0025, sh=0.01, r1=0.7, mean=[0.485, 0.456, 0.406, 0]):
        self.num_patch_l = num_patch_l
    ...
    def __call__(self, image, anns, meta):
        ...
        if self.num_patch_l >= self.num_patch_h:
            num_patch = self.num_patch_h
        else:
            num_patch = random.randint(self.num_patch_l, self.num_patch_h)

        image_prob = random.uniform(0, 1)
        if (image_prob > self.probability) or (num_patch <= 0):
            pass  # end to return
        else:
            image = np.array(image)  # H x W x C
            area = bbox[2] * bbox[3]

            sample_success, sampled_kps = sample_points_within_FG(num_patch, bbox, 'random', image[:, :, 3], thresh=0.4*255)
            if sample_success:
                for i in range(len(sampled_kps)):
                    x1, y1 = int(sampled_kps[i][0]), int(sampled_kps[i][1])
        ...
        if self.im_save:
            root = './episode_images/occluded_ims_FG/'
            if os.path.exists(root) == False:
                os.makedirs(root)
            # separate RGB image and saliency image
            R, G, B, saliency_image = image.split()
