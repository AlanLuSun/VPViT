        ...
        if trunk == 'resnet50':
            ...
            if specify_output_channel != None:
                self.end_conv1x1 = nn.Sequential(
                    nn.Conv2d(feature_dim, specify_output_channel, kernel_size=1, stride=1, padding=0, bias=False),
...
def extract_representations(features, labels, kp_mask, context_mode='soft_fiber_bilinear', sigma=13, downsize_factor=16, image_length=368, together_trans_features=None):
    ...
    :param labels: B1 x N x 2 (each row is kp (x, y), -1<= x,y < 1)
    ...
    # ====================
        ...
        representations = nn.functional.grid_sample(features, labels_normalized, mode='bilinear', align_corners=False).reshape(B1, C, N_categories)  # B1 x C x N
        # ====================
    elif context_mode == 'soft_fiber_gaussian':
        # compute heatmap which is subject to Gaussian distribution
        heatmaps = torch.zeros(B1, N_categories, H, W).cuda()
...
def feature_modulator_unsup(support_fibers, query_features, im_indexes):
    '''
    computer attentive features for each (support_fiber, query feature), where im_indexes indicate the query image which has matched query kp
    :param support_fibers: B1 x C x N, each image has N fibers
    :param query_feature:  B2 x C x H x W
    :param im_indexes: B1 x N
    return modulated features with size of B1 x N x C x H x W
    '''
    B1, C, N = support_fibers.shape
    B2, C, H, W = query_features.shape
    assert im_indexes.shape[0] == B1 and im_indexes.shape[1] == N

    inds = im_indexes.reshape(-1).long()  # (B1 * N)
    retrieved_features = query_features[inds]  # (B1 * N) x C x H x W
    retrieved_features = retrieved_features.reshape(B1, N, C, H, W)
    retrieved_features = retrieved_features * support_fibers.permute(0, 2, 1).reshape(B1, N, C, 1, 1)  # B1 x N x C x H x W
...
class GridBasedLocatorX2Covar(nn.Module):
    def __init__(self, grid_length=8, reg_uncertainty=True, cls_uncertainty=True, ureg_type=0, covar_Q_size=None, sample_times=15, negative_noise=False, sigma_computing_mode=0, alpha=1, beta=0, linear_in_channel=1024*2*2, use_tanh=True):
    ...
    def forward(self, x: torch.Tensor, training_phase=True, one_hot_grid_label=None, compute_grid_var=False):
        ...
        # grid_deviation = torch.sigmoid(grid_deviation)  # used when deviation range is 0~1, otherwise it should be commented.
        if self.use_tanh:
            grid_deviation_out = torch.tanh(grid_deviation_out)  # used when deviation range is -1~1
            # if torch.any(grid_deviation_out > 1) or torch.any(grid_deviation_out < -1):
            #     print('range overflowed!')
    ...
    # only used in transductive inference in testing phase
    def test(self, x: torch.Tensor, training_phase=True, one_hot_grid_label=None, compute_grid_var=False):
    # Note the one_hot_grid_label is not one hot code, it is the B x N query label, the value range is 0~L*L-1
        ...
        if self.reg_uncertainty == True:
            # independent var for each point B x N x 2 or covariance for each point, B x N x (2d)
            if self.ureg_type == 0 or self.ureg_type == 1:
                rho_regO = self.linear_dev_uncertainty(out)  # (B * N) * (L*L*2) or (B * N) * (L*L*2d)
                # rho_reg = rho_reg.reshape(B * N, L*L, 2)
                rho_reg = rho_regO.view(B*N*L*L, -1)  # (B*N*L*L) * 2 or (B*N*L*L) * (covar_Q_size[0] * covar_Q_size[1])
                rho_reg = rho_reg[ind_temp * L * L + retrieval_index, :]  # (B*N) * 2 or (B*N) * (covar_Q_size[0] * covar_Q_size[1])
                rho_reg = rho_reg.reshape(B, N, -1)  # B x N x 2 Or B x N x 2d
        ...
        if self.reg_uncertainty == True and compute_grid_var == False:
            ...
            return grid_class, grid_deviation, rho_reg, grid_var, grid_deviation_out, rho_regO
        else:
            return grid_class, grid_deviation, None, None
